...
def get_photo(id_vk):
    ...
    for photo in tqdm(response):
        ...
        name_v = str(name_keys)
        size_v = str(photo['sizes'][-1]['type'])
        name_k = str('file_name')
        size_k = str('size')
        info = {name_k: name_v, size_k: size_v}
        info_list.append(info)

        with open('data.json', 'w', encoding='utf-8') as file:
            json.dump(info_list[0: int(count_photos)], file)
...
if __name__ == '__main__':
    ...
    count_photos_user = input('Введите количество фотографий, которые необходимо загрузить:')

    if count_photos_user == str(''):
        count_photos = 5
    elif int(count_photos_user) > 5:
        count_photos = 5
    else:
        count_photos = int(count_photos_user)
